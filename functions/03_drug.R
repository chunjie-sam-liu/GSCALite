...
gs <- readr::read_rds("/data/GSCALite/01_gene_symbol.rds.gz")
...
drug_gdsc %>% 
  ...
  gdsc_gene_list
gdsc_gene_list %>% readr::write_rds("/data/GSCALite/Drug/gdsc_exp_spearman.rds.gz", compress = "gz")

fn_filter_drug <- function(.x, .cor = 0.2, .fdr = 0.05) {.x %>% dplyr::filter(abs(cor_sprm) > .cor, fdr < .fdr)}

gdsc_gene_list %>% 
  dplyr::filter(symbol %in% c("ATG5", "ATG7", "BECN1", "TP53")) %>% 
...
drug_ctrp %>% 
  dplyr::filter(symbol %in% gs) ->
  ctrp_gene_list
ctrp_gene_list %>% readr::write_rds("/data/GSCALite/Drug/ctrp_exp_spearman.rds.gz", compress = "gz")

t_ctrp <- readr::read_rds(file.path(tcga_path, "drug_target_ctrp.rds.gz")) %>% 
  tidyr::unnest() %>% 
  dplyr::select(drug_name, target_pathway) %>% 
  dplyr::distinct()
fn_filter_drug_ctrp <- function(.x) {.x %>% dplyr::filter(abs(cor_sprm) > 0.2, p_val < 0.05)}
ctrp_gene_list %>% 
  dplyr::filter(symbol %in% c("ATG5", "ATG7", "BECN1", "TP53")) %>% 
